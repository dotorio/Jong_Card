...
@api_view(['GET'])
def update_card_list(request, user_id):

    recommend_card_list = get_recommend_cards(user_id)
    
    return Response(recommend_card_list)
    
@api_view(['POST'])
def likes_card_toggle(request, user_id, card_id):
...
def get_recommend_cards(user_id):
    recommend_card_list = []
    ...
    if not liked_cards or not related_user_ids:
        ...
        for card in popular_cards:
            if card not in liked_cards:
                recommend_card_list.append(CardSerializer(card).data)
